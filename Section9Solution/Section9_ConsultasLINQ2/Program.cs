using Section9_ConsultasLINQ2;

namespace Section9_ConsultasLINQ2 {
    internal class Program {
        static void Main(string[] args) {
            var listaProdutos = Produto.GetProdutos();

            Console.WriteLine("\nFIRST - Localizando primeiro elemento"); 
            var primeiroProduto = listaProdutos.First();
            Console.WriteLine($"{primeiroProduto.Nome} \t{primeiroProduto.Preco:C2}");

            Console.WriteLine("\nFIRST - Localizando primeiro elemento por nome");
            try {
                var primeiraOcorrencia1 = listaProdutos.First(x => x.Nome == "Cadeira");
